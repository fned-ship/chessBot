...
int shortCastling(int board[8][8],int whiteBlack,struct Piece white[16],struct Piece black[16],int indexOfLastPiecePlayed){
    ...
    if(whiteBlack){
        ...
        for (int i = 0; i < 16; i++) {
            if (black[i].alive) {
                legalMove(black[i],board, opponentPossibleMoves, &n,white,black,0,indexOfLastPiecePlayed);
            }
        }
        for(int i=0; i<n;i++){
        ...
        for (int i = 0; i < 16; i++) {
            if (white[i].alive) {
                legalMove(white[i],board, opponentPossibleMoves, &n,white,black,0,indexOfLastPiecePlayed);
            }
        }
        for(int i=0; i<n;i++){
...
int danger(struct Piece currentPos ,int x , int y , int board[8][8],int blackWhite){ // whiteblack , who plays 
    return 0 ;
}
    ...
    int y0=currentPos.y ;
        if(currentPos.piece==-pawn || currentPos.piece==pawn ){ // pawn
            if(currentPos.piece==-pawn){ // white pawn
                if( y0-2>=0 && !danger(currentPos,x0,y0-2,board,1) && board[y0-1][x0]==0 && board[y0-2][x0]==0 && !currentPos.moveCount){
                    possibleMoves[*n].x=x0;possibleMoves[*n].y=y0-2;
                    possibleMoves[*n].x0=x0;possibleMoves[*n].y0=y0;possibleMoves[*n].promotion=0;
                    (*n)++ ;
                }
                if( y0-1>=0 && !danger(currentPos,x0,y0-1,board,1) && board[y0-1][x0]==0 ){
                ...
                if(x0-1>=0 && y0-1>=0 && !danger(currentPos,x0-1,y0-1,board,1) && board[y0-1][x0-1]>0){
                    if(y0==1){
                        //queen
                ...
                if(x0+1<8 && y0-1>=0 && !danger(currentPos,x0+1,y0-1,board,1) && board[y0-1][x0+1]>0){
                    if(y0==1){
                        //queen
                        possibleMoves[*n].x=x0+1;possibleMoves[*n].y=y0-1;
                        possibleMoves[*n].x0=x0;possibleMoves[*n].y0=y0;possibleMoves[*n].promotion=-queen;
                ...
                //en passent
                if(x0-1>=0 && y0-1>=0 && !danger(currentPos,x0-1,y0-1,board,1) && board[y0-1][x0-1]==0 && board[y0][x0-1]==pawn && getIndexOfPawn(x0-1,0)==indexOfLastPiecePlayed && black[getIndexOfPawn(x0-1,0)].moveCount==1 ){
                    possibleMoves[*n].x=x0-1;possibleMoves[*n].y=y0-1;
                    possibleMoves[*n].x0=x0;possibleMoves[*n].y0=y0;possibleMoves[*n].promotion=0;
                    (*n)++ ;
                }
                if(x0+1<8 && y0-1>=0 && !danger(currentPos,x0+1,y0-1,board,1) && board[y0-1][x0+1]==0 && board[y0][x0+1]==pawn && getIndexOfPawn(x0+1,0)==indexOfLastPiecePlayed && black[getIndexOfPawn(x0+1,0)].moveCount==1 ){
                    possibleMoves[*n].x=x0+1;possibleMoves[*n].y=y0-1;
                    possibleMoves[*n].x0=x0;possibleMoves[*n].y0=y0;possibleMoves[*n].promotion=0;
                    (*n)++ ;
                }
            }else{
                if( y0+2<8 && !danger(currentPos,x0,y0+2,board,0) && board[y0+1][x0]==0 && board[y0+2][x0]==0 && !currentPos.moveCount){
                    possibleMoves[*n].x=x0;possibleMoves[*n].y=y0+2;
                    possibleMoves[*n].x0=x0;possibleMoves[*n].y0=y0;possibleMoves[*n].promotion=0;
                    (*n)++ ;
                }
                if( y0+1<8 && !danger(currentPos,x0,y0+1,board,0) && board[y0+1][x0]==0 ){
                    if(y0==6){
                        //queen
                        possibleMoves[*n].x=x0;possibleMoves[*n].y=y0+1;
                        possibleMoves[*n].x0=x0;possibleMoves[*n].y0=y0;possibleMoves[*n].promotion=queen;
                ...
                if(x0-1>=0 && y0+1<8 && !danger(currentPos,x0-1,y0+1,board,0) && board[y0+1][x0-1]<0){
                    if(y0==6){
                        //queen
                        possibleMoves[*n].x=x0-1;possibleMoves[*n].y=y0+1;
                        possibleMoves[*n].x0=x0;possibleMoves[*n].y0=y0;possibleMoves[*n].promotion=queen;
                ...
                if(x0+1<8 && y0+1<8 && !danger(currentPos,x0+1,y0+1,board,0) && board[y0+1][x0+1]<0){
                    if(y0==6){
                        //queen
                        possibleMoves[*n].x=x0+1;possibleMoves[*n].y=y0+1;
                        possibleMoves[*n].x0=x0;possibleMoves[*n].y0=y0;possibleMoves[*n].promotion=queen;
                ...
                //en passent
                if(x0-1>=0 && y0+1<8 && !danger(currentPos,x0-1,y0+1,board,1) && board[y0+1][x0-1]==0 && board[y0][x0-1]==-pawn && getIndexOfPawn(x0-1,1)==indexOfLastPiecePlayed && white[getIndexOfPawn(x0-1,1)].moveCount==1  ){
                    possibleMoves[*n].x=x0-1;possibleMoves[*n].y=y0+1;
                    possibleMoves[*n].x0=x0;possibleMoves[*n].y0=y0;possibleMoves[*n].promotion=0;
                    (*n)++ ;
                }
                if(x0+1<8 && y0+1<8 && !danger(currentPos,x0+1,y0+1,board,1) && board[y0+1][x0+1]==0 && board[y0][x0+1]==-pawn && getIndexOfPawn(x0+1,1)==indexOfLastPiecePlayed && white[getIndexOfPawn(x0+1,1)].moveCount==1 ){
                    possibleMoves[*n].x=x0+1;possibleMoves[*n].y=y0+1;
                    possibleMoves[*n].x0=x0;possibleMoves[*n].y0=y0;possibleMoves[*n].promotion=0;
                    (*n)++ ;
                }
            ...
            int i=x0-1;
            while(i>=0){
                if(!danger(currentPos,i,y0,board,currentPos.piece<0) && board[y0][i]==0){
                    possibleMoves[*n].x=i;possibleMoves[*n].y=y0;
                    possibleMoves[*n].x0=x0;possibleMoves[*n].y0=y0;possibleMoves[*n].promotion=0;
                    (*n)++ ;
                }else{
                    if(!danger(currentPos,i,y0,board,currentPos.piece<0) && currentPos.piece*board[y0][i]<0){
                        possibleMoves[*n].x=i;possibleMoves[*n].y=y0;
                        possibleMoves[*n].x0=x0;possibleMoves[*n].y0=y0;possibleMoves[*n].promotion=0;
                        (*n)++ ;
                        break;
            ...
            i=y0+1;
            while(i<8){
                if(!danger(currentPos,x0,i,board,currentPos.piece<0) && board[i][x0]==0){
                    possibleMoves[*n].x=x0;possibleMoves[*n].y=i;
                    possibleMoves[*n].x0=x0;possibleMoves[*n].y0=y0;possibleMoves[*n].promotion=0;
                    (*n)++ ;
                }else{
                    if(!danger(currentPos,x0,i,board,currentPos.piece<0) && currentPos.piece*board[i][x0]<0){
                        possibleMoves[*n].x=x0;possibleMoves[*n].y=i;
                        possibleMoves[*n].x0=x0;possibleMoves[*n].y0=y0;possibleMoves[*n].promotion=0;
                        (*n)++ ;
                        break;
            ...
            int i=1;
            while(x0+i<8 && y0+i<8){
                if(!danger(currentPos,x0+i,y0+i,board,currentPos.piece<0) && board[y0+i][x0+i]==0){
                    possibleMoves[*n].x=x0+i;possibleMoves[*n].y=y0+i;
                    possibleMoves[*n].x0=x0;possibleMoves[*n].y0=y0;possibleMoves[*n].promotion=0;
                    (*n)++ ;
                }else{
                    if(!danger(currentPos,x0+i,y0+i,board,currentPos.piece<0) && currentPos.piece*board[y0+i][x0+i]<0){
                        possibleMoves[*n].x=x0+i;possibleMoves[*n].y=y0+i;
                        possibleMoves[*n].x0=x0;possibleMoves[*n].y0=y0;possibleMoves[*n].promotion=0;
                        (*n)++ ;
                        break;
            ...
            for(int i=0;i<8;i++){
                if(!danger(currentPos,posMvs[i][0],posMvs[i][1],board,currentPos.piece<0) && posMvs[i][1]<8 && posMvs[i][1]>=0 && posMvs[i][0]<8 && posMvs[i][0]>=0  && currentPos.piece*board[posMvs[i][1]][posMvs[i][0]]<=0 ){
...
float evaluatePosition(int board[8][8] ,struct Piece white[16],struct Piece black[16],int indexOfLastPiecePlayed){
    ...
    for (int i = 0; i < 16; i++) {
        if (white[i].alive) {
            legalMove(white[i], board, whitePossibleMoves, &n1,white,black,1,indexOfLastPiecePlayed);
        }
    }
    for (int i = 0; i < 16; i++) {
        if (black[i].alive) {
            legalMove(black[i], board, blackPossibleMoves, &n2,white,black,1,indexOfLastPiecePlayed);
        }
    }
    s+=n2/2560 ;
    s-=n1/2560 ;
    ...
    if (maximizingPlayer) {
        ...
        for (int i = 0; i < 16; i++) {
            if (tempBlack[i].alive) {
                legalMove(tempBlack[i], tempBoard, possibleMoves, &n,tempWhite,tempBlack,1,indexOfLastPiecePlayed);
            }
        }
        ...
        for (int i = 0; i < 16; i++) {
            if (tempWhite[i].alive) {
                legalMove(tempWhite[i], tempBoard, possibleMoves, &n,tempWhite,tempBlack,1,indexOfLastPiecePlayed);
            }
        }
...
int main(){
    FILE *file = fopen("/home/yousseffned/Desktop/Xturing/chessBot/coordinate.txt", "r+");
    ...
    char UserMove[12]="y0-y0-0-0-0";
    char move[12]="y0-y0-0-0-0";
    int m=0,n=0;
    ...
    // game
    while(1){
        file = fopen("/home/yousseffned/Desktop/Xturing/chessBot/coordinate.txt", "r+");
        //get the coordinate from user
        fscanf(file,"%s",UserMove);
        gameOver=getGameOver(UserMove);
        if(gameOver){
            break;
        }
        if(strcmp(UserMove,move)){
            // get the coordinate of move
            strcpy(move,UserMove);
            x0=getx0(move);
            y0=gety0(move);
            x=getx1(move);
            y=gety1(move);
            ...
            fprintf(file,"%c%d-%c%d-%d-%d-0\n", 'a'+x0, 8-y0, 'a'+x, 8-y,promotion,isKnightPromotion);
            //print game
            printGame(board);
            fseek(file, 0, SEEK_SET);

            sprintf(move,"%c%d-%c%d-%d-%d-0", 'a'+x0, 8-y0, 'a'+x, 8-y,promotion,isKnightPromotion);
            sprintf(UserMove,"%c%d-%c%d-%d-%d-0", 'a'+x0, 8-y0, 'a'+x, 8-y,promotion,isKnightPromotion);
            printf("%s ## %s\n",move,UserMove);
        }
